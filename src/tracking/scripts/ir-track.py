...
class IRTracking:
    ...
    def update_marker_thread(self, gray_img, dt, marker):
        ...
        if mean >= 0.1:                                   #TODO depends on halfsize and is critical if ledpoint is to small!!! also depends on camera ExposureTime!!!
        ...
        if self.markers is None:
            ...
            for point in points:
                ...
                for cpoint in points_cleared:
                     if abs(point[0]-cpoint[0]) < 50 and abs(point[1]-cpoint[1]) < 50:
                         add=False
                if add:
            ...
            for marker in self.markers:
                ...
                #t.start()
                self.update_marker_thread(gray_img,dt, marker)
    ...
    def pubish_marker(self):
        ...
        for pub in self.publisher_list:
            tmp.x=self.markers[i]['pos'][0]
            tmp.y=self.markers[i]['pos'][1]
            i+=1
            pub.publish(tmp)
    ...
    def to_topview(self, homography, img, roi_points=None):
        ...
        return cv2.warpPerspective(img, m , topview_size)
